...
This setup was tested on a Samsung Galaxy S8 and a Samsung Galaxy S20.
...
You do not need to open any inbound ports on either the phone or the router. A Cloudflare Tunnel establishes an outbound connection from the phone to Cloudflare.

> [!WARNING] Availability
> This setup is useful for personal development, experiments, and occasional remote access. It is not equivalent to a conventional production server. Android may terminate Termux because of battery-management policies, memory pressure, or system updates; a reboot will also stop it.
...
Throughout this guide, `ssh.example.com` is a placeholder for the hostname you want to use, and `<TUNNEL_UUID>` is a placeholder for the UUID generated by Cloudflare.
...
Do not mix Termux apps or plugins installed from different sources. For example, do not install Termux from F-Droid and Termux:Boot from GitHub because the packages use different signing keys. The Termux project currently treats the Google Play version as a separate, experimental build.

After installation, open Termux and let it complete its initial setup.

## 2. Disable battery restrictions

Open the Android app settings for Termux and set its battery usage to **Unrestricted** or exclude it from battery optimization.

The exact location varies by Android version and device manufacturer. On Samsung devices, it is usually under:
...
A wake lock helps keep the processor awake while the screen is off, but it cannot prevent Android from terminating Termux.
...
Inside Ubuntu, refresh the package database and upgrade the installed packages:
...
Install OpenSSH and the other tools required for this guide:
...
* `openssh-server`: runs the SSH server.
* `nano`: edits text files.
* `curl`: downloads the Cloudflare signing key.
* `ca-certificates`: enables HTTPS certificate verification.
...
SSH key authentication is safer than password authentication for the Ubuntu root account.

Run the appropriate command on the computer that will connect to the phone, not inside Termux.
...
The command creates two files in your `.ssh` directory: `android-server` and `android-server.pub`.

The first file is the private key. Do not share it.

The `.pub` file is the public key and can safely be copied to the phone. `ssh-keygen` is the standard OpenSSH utility for creating authentication keys.
...
Copy the complete line. It should begin with `ssh-ed25519`.
...
This ensures that the account is unlocked. The next section disables password authentication, so this password will not be accepted over SSH.

> [!NOTE] Ubuntu root versus Android root
> The root user in this guide controls the Ubuntu PRoot environment. This does not grant the Ubuntu environment root access to the underlying Android operating system.
...
* Allows root login using public-key authentication only.

`PermitRootLogin prohibit-password` allows root login using public-key authentication while disabling password and keyboard-interactive authentication for root.
...
OpenSSH may display `without-password`, a deprecated alias for `prohibit-password`.
...
Still inside Ubuntu, create the APT keyring directory:
...
Using the `any` suite avoids tying the Cloudflare package source to a specific Ubuntu release name. Cloudflare publishes both stable and nightly package channels; this guide uses the stable channel.
...
`cloudflared` will attempt to open a browser. Inside the PRoot environment, the command may display a URL instead.
...
4. Select the domain for `ssh.example.com`.
5. Return to Termux after Cloudflare confirms authorization.

The command creates an account certificate at:
...
Create the Access application **before publishing the DNS route**, as Cloudflare recommends. This prevents the hostname from being briefly available without an Access policy. Access applications deny access by default, and a user must match an **Allow** policy to connect.
...
Enter the exact email address you will use for authentication.

Do not configure an Allow policy with the **Everyone** selector unless the application is intentionally available to every user who can authenticate.

For the authentication method, select one of the following:

* Cloudflare One-time PIN.
* An identity provider already configured in your Zero Trust account.

Cloudflare One-time PIN sends a login code to an approved email address without requiring a separate third-party identity provider integration.

Choose an appropriate session duration and create the application.

The hostname configured for the Access application must exactly match:

```text
ssh.example.com
```

In Cloudflare's current application workflow, the policy appears directly under **Access policies** in the self-hosted application configuration.
...
The final catch-all rule is required so requests for unmatched hostnames receive an HTTP `404` response instead of being forwarded to the SSH service.

This guide uses the TCP service type (`tcp://127.0.0.1:2022`) because the client connects with `cloudflared access tcp`.
...
Alternatively, run the tunnel by name:
...
The tunnel is available only while all of the following conditions are met:
...
Cloudflare's method for arbitrary TCP connections requires `cloudflared` on both the server and the client.
...
The relay forwards connections through Cloudflare Access and the tunnel to the following address inside Ubuntu on the phone:

```text
127.0.0.1:2022
```

When the relay starts, `cloudflared` opens a browser window for authentication. Depending on your Access configuration, sign in through the identity provider or enter the one-time PIN sent to your email. Cloudflare documents this browser-based authentication flow for `cloudflared access tcp`.
...
Use the same new port in the SSH command and VS Code configuration.
...
Before accepting it, display the expected Ed25519 host-key fingerprint from Ubuntu on the phone:

```bash
ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub
```

Compare the two fingerprints. If they match, enter:

```text
yes
```

Enter the passphrase for the SSH private key if prompted.

You should then receive an Ubuntu shell on the Android phone.

The `HostKeyAlias` option tells OpenSSH to store and look up the server's host key under `ssh.example.com` instead of `[127.0.0.1]:9000`.
...
You can now start the server and tunnel from Termux with:
...
Install Termux:Boot from the same source that you used to install Termux.
...
Open the Termux:Boot application once after installing it, as the project's documentation recommends. This initializes its boot integration.

Termux:Boot runs executable scripts located in:
...
> Automatic startup depends on Android's background-execution rules and any additional restrictions imposed by the device manufacturer. A boot script does not guarantee uninterrupted availability. Some devices may require the phone to be unlocked once after reboot.

## 21. Stop the server

When `cloudflared` is running in the foreground, press <kbd>Ctrl</kbd>+<kbd>C</kbd> to stop the tunnel.
...
Update the Ubuntu packages, including `cloudflared`:
...
* `/root/.ssh` has permissions set to `700`.
* `/root/.ssh/authorized_keys` has permissions set to `600`.
* The hostname configured for the Cloudflare Access application exactly matches the tunnel and client hostname.
* The Access **Allow** policy contains only authorized identities and does not use the **Everyone** selector.
* `/root/.cloudflared` has permissions set to `700`.
* `cert.pem`, `config.yml`, and the tunnel JSON file have permissions set to `600`.
...
Confirm that the public key in `authorized_keys` matches the public key stored in the client's `android-server.pub` file.
...
Confirm that the three hostname values match and that the local service is configured exactly as shown:
...
Make sure the terminal running `cloudflared access tcp` is still open.
...
A host-key change may be legitimate if Ubuntu was reinstalled or its host keys were deliberately regenerated. It can also indicate that the connection is reaching a different server.

Verify the reason before removing the stored key.

After intentionally rebuilding the Ubuntu environment, remove the old host-key entry for the alias only after confirming the change:
...
Confirm that Termux's battery usage setting in Android is set to **Unrestricted**.

Also check manufacturer-specific settings such as sleeping apps, limits on background usage, and automatic memory cleanup.

## 25. Limitations

Ubuntu is running inside a PRoot environment rather than as a conventional virtual machine or native Android service. The setup therefore depends on Termux remaining active.

The tunnel's public TCP mode streams TCP over a WebSocket connection. For connections that must remain active for long periods, Cloudflare recommends its Client-to-Tunnel architecture instead.
